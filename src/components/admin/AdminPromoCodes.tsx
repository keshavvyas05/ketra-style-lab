...
import { allPromoCodes } from "@/data/adminMockData";

const AdminPromoCodes = () => {
  const [codes, setCodes] = useState(allPromoCodes);
  const [filter, setFilter] = useState("all");
  const [showCreate, setShowCreate] = useState(false);
  const [newCode, setNewCode] = useState({ code: "", type: "Free Try-ons", value: "", maxUses: 100, expiry: "", active: true });

  const filtered = codes.filter((c) => {
    if (filter === "active") return c.active;
    if (filter === "expired") return !c.active;
    return true;
  });

  const totalRedemptions = codes.reduce((a, c) => a + c.used, 0);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-card/70 backdrop-blur-sm border border-border/40 rounded-2xl p-5 text-center">
          ...
          <p className="font-display text-2xl font-extrabold">{codes.filter((c) => c.active).length}</p>
        </div>
      </div>
        ...
        <div className="bg-card/70 backdrop-blur-sm border border-accent/30 rounded-2xl p-6">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            ...
            <select value={newCode.type} onChange={(e) => setNewCode({ ...newCode, type: e.target.value })} className="bg-secondary/40 border border-border/40 rounded-xl px-3 py-2.5 font-body text-sm text-foreground focus:outline-none">
              <option>Free Try-ons</option>
              <option>Discount</option>
            </select>
            <input value={newCode.value} onChange={(e) => setNewCode({ ...newCode, value: e.target.value })} placeholder="Value (e.g. 25% or 5 try-ons)" className="bg-secondary/40 border border-border/40 rounded-xl px-3 py-2.5 font-body text-sm text-foreground placeholder:text-muted-foreground focus:outline-none" />
            ...
            <div className="flex gap-2">
              <button onClick={() => { setCodes([...codes, { ...newCode, id: codes.length + 1, used: 0, created: new Date().toISOString().split("T")[0] }]); setShowCreate(false); }} className="flex-1 py-2.5 rounded-xl bg-accent text-accent-foreground font-display text-xs tracking-[0.1em] uppercase">Create</button>
              ...
              {filtered.map((c) => (
                <tr key={c.id} className="border-b border-border/20 hover:bg-secondary/20 transition-colors">
                  <td className="py-3 px-3">
                    <span className="px-3 py-1 rounded-lg bg-accent/15 font-display text-sm font-bold tracking-[0.1em] text-accent">{c.code}</span>
                  </td>
                  <td className="py-3 px-3 font-body text-sm text-muted-foreground">{c.type}</td>
                  <td className="py-3 px-3 font-display text-sm font-bold">{c.value}</td>
                  <td className="py-3 px-3">
                    <div className="flex items-center gap-2">
                      <span className="font-body text-xs text-muted-foreground">{c.used}/{c.maxUses}</span>
                      <div className="w-16 h-1.5 rounded-full bg-secondary overflow-hidden">
                        <div className="h-full rounded-full bg-accent" style={{ width: `${(c.used / c.maxUses) * 100}%` }} />
                      </div>
                    </div>
                  </td>
                  <td className="py-3 px-3 font-body text-xs text-muted-foreground">{c.expiry}</td>
                  <td className="py-3 px-3">
                    <button
                      onClick={() => setCodes((prev) => prev.map((p) => p.id === c.id ? { ...p, active: !p.active } : p))}
                      className={`px-2.5 py-0.5 rounded-full font-display text-[10px] tracking-[0.1em] uppercase font-bold cursor-pointer ${c.active ? "bg-[hsl(160_60%_45%_/_0.15)] text-[hsl(160_60%_45%)]" : "bg-[hsl(0_60%_55%_/_0.15)] text-[hsl(0_60%_55%)]"}`}
                    >
                      {c.active ? "Active" : "Expired"}
                    </button>
                  </td>
                  <td className="py-3 px-3 font-body text-xs text-muted-foreground">{c.created}</td>
                </tr>
              ))}
            </tbody>
          </table>
